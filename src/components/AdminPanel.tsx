          ...
          <div className={`space-y-6 p-6 rounded-3xl ${styles.card}`}>
              ...
              <div className="space-y-2 max-h-[60vh] overflow-y-auto pr-2">
                  {filteredManageSongs.map(song => (
                      <div key={song.id} className={`flex items-center justify-between p-3 rounded-xl border ${styles.border} bg-white/5`}>
                          <div className="flex-1 min-w-0 mr-4">
                              <h4 className={`font-bold truncate ${styles.textMain}`}>{song.title}</h4>
                              <p className={`text-xs truncate ${styles.textSec}`}>{song.artist}</p>
                          </div>
                          <div className="flex items-center gap-2">
                              <button 
                                  onClick={() => handleTogglePin(song)}
                                  className={`p-2 rounded-lg transition-colors ${song.isPinned ? 'bg-yellow-500/20 text-yellow-400' : 'bg-white/5 text-gray-400 hover:bg-white/10'}`}
                                  title={song.isPinned ? "Unpin" : "Pin to top"}
                              >
                                  <Pin size={16} fill={song.isPinned ? "currentColor" : "none"} />
                              </button>
                              <button 
                  ...
                  {filteredManageSongs.length === 0 && (
                      <div className={`text-center py-8 ${styles.textSec}`}>No tracks found.</div>
                  )}
              </div>
          </div>
      )}
      
      {statusMsg && <div className={`mt-4 p-4 rounded-xl ${styles.card} ${styles.textMain}`}>{statusMsg}</div>}
    </div>
  );
};
